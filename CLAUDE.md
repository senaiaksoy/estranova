...
- [ ] **Yapılandırılmış veri (JSON-LD):** Her yayın makalesinde `MedicalWebPage` + `Article` + `BreadcrumbList` + `FAQPage` schema'sı var; `author.Person` writers.ts'ten geliyor, `reviewedBy.Person` tıbbi editör (Doç. Dr. Senai Aksoy), `articleSection` + `sectionPath` kategoriye uygun. Helper: `src/utils/article-schema.ts → buildArticleSchemas()`.
