...
- [ ] **Makale görsel yayın kapısı:** Yeni/onaylı makale yayına alınırken özel makale görseli yoksa editörden görsel istenir. Verilen makale görseli varsayılan olarak yalnızca `ArticleAuthorBlock imageSrc` (makale içi Dosya görseli) ve `articleCardImageByRoute` (alt menü/kart görseli) alanlarına bağlanır; `submenuHeroByRoute` / üst hero görseli kullanıcı açıkça istemedikçe değiştirilmez.
