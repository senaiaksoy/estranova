...
def test_portfolio_report_shows_projection_table():
    provider = StaticPriceProvider(
        {
            "YAY": PriceSnapshot("YAY", 100.0, "TRY", "test", "2026-07-04"),
            "YFT": PriceSnapshot("YFT", 1.0, "TRY", "test", "2026-07-04"),
    ...
    projected = value_holdings(default_user_holdings(), provider)

    report = render_portfolio_report(current, projected_valuation=projected)
    projection_section = report.split("### Projeksiyon", 1)[1].split(
        "## Veri Uyarıları", 1
    )[0]

    assert "YAY / YFAY1" in projection_section
    assert "YFT / YPT" in projection_section
    assert "Fiziki altın" in projection_section
